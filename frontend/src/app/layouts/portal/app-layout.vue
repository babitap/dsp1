...
import { ColorThemes, AvailableColorThemes } from './services/themes-config'
import {
...
export default {
  ...
  created () {
    this.setTheme(ColorThemes.DEFAULT)
  ...
  methods: {
    setTheme (themeName) {
      const theme = AvailableColorThemes.find(item => item.name === themeName)
